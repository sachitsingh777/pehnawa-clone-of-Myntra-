...
const Women = () => {
  ...
  const getmen=()=>{
     let apiurl;
    if(orderby){
     apiurl=`http://localhost:8080/women?_sort=${sort}&order=${orderby}`
    }else if(filterby){
     apiurl=`http://localhost:8080/women?brand=${filterby}&_sort=${sort}&order=${orderby}`
    }else{
     apiurl=`http://localhost:8080/women`
    }
